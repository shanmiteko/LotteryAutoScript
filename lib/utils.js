...
const utils = {
  ...
  checkVersion(version) {
    return version
      .match(/\d.*/)[0]
      .split('.')
      .reduce((a, v, i) => a + 0.01 ** i * Number(v), 0)
  },
  /**
   * 安全的将JSON字符串转为对象
   * 超出精度的数转为字符串
  ...
  *infiniteNumber() {
  ...
  judge(text, key_words) {
    return key_words.reduce((acc, word) => {
      word.startsWith('~') ? RegExp(word.slice(1)).test(text) && (acc = false) : RegExp(word).test(text) && (acc = true)
      return acc
    }, false)
  },
  /**
  ...
  log: {
    _level: 3,
    _colors: [chalk.hex('#64B3FF'), chalk.grey, chalk.hex('#FFA500'), chalk.hex('#0070BB'), chalk.hex('#48BB31'), chalk.hex('#BBBB23'), chalk.hex('#FF0006')],
