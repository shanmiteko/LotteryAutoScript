...
class Monitor extends Public {
    ...
    async startLottery() {
        ...
        switch (status) {
            case 0:
                ...
                break;
            case 1:
                eventBus.emit('Turn_off_the_Monitor', '访问频繁')
                break;
            case 2:
                eventBus.emit('Turn_off_the_Monitor', '账号异常')
                break;
            default:
                break;
        }
    ...
    async go(option) {
        ...
        if (!this.isAttentionErr) {
            uid.forEach(async (one_uid) => {
                if (typeof one_uid === 'number') {
                    isBan = await BiliAPI.autoAttention(uid)
                    if (isBan) {
        ...
        if (isBan) return isBan;
        if (this.isAttentionErr && uid.length !== 0) return 1;
        if (await BiliAPI.autolike(dyid)) return 1;
        if (await BiliAPI.autoRelay(GlobalVar.get("myUID"), dyid, relay_chat, ctrl)) return 1;
