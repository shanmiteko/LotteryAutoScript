...
/** 中奖推送限制(小时) 如需修改请在添加在env.bat中*/
const PUSH_LIMIT_TIME = Number(process.env.PUSH_LIMIT_TIME) || 2;
...
async function isMe() {
    ...
    if (MyAtInfo.length) {
        ...
        MyAtInfo.forEach(async AtInfo => {
            ...
            if ((Date.now() / 1000 - at_time) / 3600 < PUSH_LIMIT_TIME) {
    ...
    if (MySession.data.length) {
        ...
        do {
            for (const Session of MySession.data) {
                ...
                /**两小时内 */
                if ((Date.now() / 1000 - timestamp) / 3600 < PUSH_LIMIT_TIME) {
                    if (/中奖|填写|写上|提供|收货地址|支付宝账号|收款码|激活码|大会员/.test(content)) {
...
async function checkCookie(num) {
    if (await BiliAPI.getMyinfo()) {
        ...
        await sendNotify('动态抽奖出错-登录失败', `COOKIE${num} 已失效 UID:${GlobalVar.myUID}`)
        return false;
    }
}
