...
// ====================================PUSH_PLUS推送加======================================
//官方https://www.pushplus.plus/
let PUSHPLUS_Plus_TOKEN = ''
let PUSHPLUS_Plus_Topic = ''
...
if (process.env.PUSHPLUS_Plus_TOKEN) {
  PUSHPLUS_Plus_TOKEN = process.env.PUSHPLUS_Plus_TOKEN
}
...
async function Pushplus(title, content) {
  return new Promise(resolve => {
    if (PUSHPLUS_Plus_TOKEN) {
      try {
        send({
          method: 'POST',
          url: 'http://www.pushplus.plus/send/',
          contents: {
            token: PUSHPLUS_Plus_TOKEN,
            title: title,
            content: content,
            topic: PUSHPLUS_Plus_Topic,
            template: 'html'
          },
          config: {
            retry: false
          ...
          success: () => {
            log.info('发送通知', 'Pushplus 发送通知消息成功')
            resolve()
          },
          failure: err => {
            log.error('发送通知', 'Pushplus 发送通知调用API失败！！' + err)
            resolve()
          }
        })
      } catch (e) {
        log.debug('发送通知', '您未提供Pushplus推送所需的PUSHPLUS_Plus_TOKEN，取消Pushplus推送消息通知')
        resolve()
      }
    }
  })
