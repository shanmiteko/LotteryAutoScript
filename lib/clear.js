...
async function unFollow () {
    ...
    for (let index = 1; index < 42; index++) {
        ...
        const cancelFollowUIDList = uids.filter(uid => followWhiteList.split(',').indexOf(String(uid)) === -1);
        if (cancelFollowUIDList.length) rmup.push(...cancelFollowUIDList);
        if (cancelFollowUIDList.length === 0) break;
    }
