...
const Base = {
    ...
    getHiToKoTo() {
        return new Promise(resolve => {
            HttpRequest({
                ...
                success: res => {
                    const { hitokoto } = Base.strToJson(res.body);
                    if (hitokoto) {
                        resolve(hitokoto)
                    } else {
    ...
    getZhihuBillBoard() {
        return new Promise((resolve) => {
            HttpRequest({
                ...
                success: res => {
                    const $ = load(res.body);
                    const ret = Array.from($('.HotList-itemTitle').map((_, ele) => {
                        return $(ele).text()
                    }))
                    if (ret.length) {
                        resolve(ret)
                    } else {
                        resolve('‍[tv_doge]‍‍[tv_doge]‍‍[tv_doge]‍')
                    }
                },
                failure: err => {
                    console.log(err)
    ...
    writeDyidFile: (dyid) => {
        const fpath = path.join('./lib', 'dyid.txt');
        return fs.writeFileSync(fpath, dyid, { flag: 'a' })
    }
};
