...
const BiliAPI = {
                ...
                success: responseText => {
                    ...
                    if (res.code === 0) {
                        ...
                        const data = sessions.map(session => {
                            const { session_ts, last_msg } = session;
                            const { content, timestamp, sender_uid} = last_msg;
                            return { session_ts, content, timestamp, sender_uid}
                        })
     ...
     * @typedef DraftList
     * @property {Array<Draft>} drafts
     * @property {Page} page
     * @param {number} pn 页数
     * @returns {Promise<DraftList>} 
     * 错误返回 {}
     */
    showDraftList: (pn) => {
        return new Promise((resolve) => {
            Ajax.get({
                ...
                hasCookies: true,
                success: responseText => {
                    const { artlist } = Base.strToJson(responseText);
                    if (typeof artlist === 'object' && artlist !== null) {
                        const { drafts, page } = artlist;
                        resolve({ drafts, page })
                    } else {
                        resolve({});
                    }
        ...
        return new Promise((resolve) => {
            Ajax.post({
                ...
                data: {
                    title: '转发过的动态',
                    content: `<p>${text}</p>`,
                    summary: '此处存放转发过的动态的id',
                    csrf: GlobalVar.csrf,
                    aid
                },
                success: responseText => {
                    const { code, data } = Base.strToJson(responseText);
                    if (code === 0) {
                        console.log('[更新专栏]成功');
                        resolve(data.aid);
                    } else {
        ...
        return new Promise((resolve) => {
            Ajax.get({
                ...
                success: responseText => {
                    const { code, data } = Base.strToJson(responseText);
                    if (code === 0) {
                        console.log('[获取专栏草稿]成功');
                        const { content = '<p></p>' } = data;
                        resolve(content.slice(3, -4));
                    } else {
                        console.log('[获取专栏草稿]失败');
                        resolve('');
                    }
                }
            })
